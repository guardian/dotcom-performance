...
class ArticleUrls(key: String) {
  ...
  def getSinglePage(urlString: String): (Option[ContentFields],String) = {
    ...
    try {
      ...
      val apiResponse = contentApiClient.getResponse(searchQuery)
      val returnedResponse = Await.result(apiResponse, (20, SECONDS))
      val resultAndUrl = (returnedResponse.content.get.fields, returnedResponse.content.get.webUrl)
      ...
      if(resultAndUrl._1.isEmpty){
        ...
        val emptyContentFields: (Option[ContentFields], String) = (Option(makeContentStub(Option(urlString), None, Option(false))), urlString)
        emptyContentFields
      } else {
